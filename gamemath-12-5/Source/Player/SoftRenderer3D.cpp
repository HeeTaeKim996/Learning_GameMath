...
void SoftRenderer::Render3D()
{
	...
	static std::array<Plane, 6> frustumPlanes = {
		Plane(Vector3(0, pCos, pSin), 0),	// 상평면
		Plane(Vector3(0, -pCos, pSin), 0),	// 하평면
		Plane(Vector3(-pCos, 0, pSin), 0),	// 좌평면
		Plane(Vector3(pCos, 0, pSin), 0), // 우평면
		Plane(Vector3::UnitZ, -nearZ), // 근평면    @@@@@@@@@@@@@@@ 틀린 것 같은데.. 카메라 앞에 근평면이 있으니, -nearZ여야 하는데
		Plane(-Vector3::UnitZ, -farZ) // 원평면
	};
	...
	std::array<Plane, 6> frustumPlanes = {
		Plane(pMatrix[0] - pMatrix[3]),		// 우측면
		Plane(-pMatrix[0] - pMatrix[3]),	// 좌측면
		Plane(pMatrix[1] - pMatrix[3]),		// 상측면
		Plane(-pMatrix[1] - pMatrix[3]),	// 하측면
		Plane(pMatrix[2] - pMatrix[3]),		// 원평면
		Plane(-pMatrix[2] - pMatrix[3])		// 근평면
	};
...
#if  defined(EX_13_1) || defined(EX_13_3)
	...
	r.PushStatisticText("Culled GameObject : " + std::to_string(culledObjects));
	r.PushStatisticText("Rendered GameObject : " + std::to_string(renderObjects));

...
#if 0 // // WindowsRSI::CohenSutherlandLineClip 함수 원본
...
#endif // WindowsRSI::CohenSutherlandLineClip 함수 원본
...
#if 0 // WindowsRSI::DrawLine 함수 원본
...
#endif // WindowsRSI::DrawLine 함수 원본
...
#if 1 // WindowsRSI::DrawLine 함수 연습1
void WindowsRSI::DrawLine(const Vector2& InStartPos, const Vector2& InEndPos, const LinearColor& InColor)
{
	Vector2 clippedStart = InStartPos;
	Vector2 clippedEnd = InEndPos;
...
#endif // WindowsRSI::DrawLine 함수 연습



#if 1 // WindowsRSI::TestRegion 함수 연습
int WindowsRSI::TestRegion(const Vector2& InVectorPos, const Vector2& InMinPos, const Vector2& InMaxPos)
{
	int result = 0;
	if (InVectorPos.X < InMinPos.X)
...
#endif // WindowsRSI::TestRegion 함수 연습

#if 1 // WindowsRSI::CohenSutherlandLineClip 함수 연습
bool WindowsRSI::CohenSutherlandLineClip(Vector2& InOutStartPos, Vector2& InOutEndPos, const Vector2& InMinPos,
	const Vector2& InMaxPos)
{
	int startTest = TestRegion(InOutStartPos, InMinPos, InMaxPos);
	int endTest = TestRegion(InOutEndPos, InMinPos, InMaxPos);
	...
	while (true)
	{
		...
		else
		{
			...
			if (currentTest < 0b100)
			{
				...
				{
					clippedPosition.Y = InOutStartPos.Y + height / width * (clippedPosition.X - InOutStartPos.X);
				}
			}
			else
			{
				...
				{
					clippedPosition.X = InOutStartPos.X + height / width * (clippedPosition.Y - InOutStartPos.Y);
				}
			}
...
#endif // WindowsRSI::CohenSutherlandLineClip 함수 연습
...
#if defined(EX_12_2) || defined(EX_12_3)
	FORCEINLINE Matrix4x4 CameraObject::GetPerspertiveViewMatrix() const;
	float _NearZ = 5.5f;
	float _FarZ = 5'000.f;
#endif
...
FORCEINLINE Matrix4x4 CameraObject::GetPerspertiveViewMatrix() const
{
	...
	float invNF = 1.f / (_NearZ - _FarZ);
	float k = (_NearZ + _FarZ) * invNF;
	float l = 2.f * _NearZ * _FarZ * invNF;
...
#else
#if 0 // 공붕 원본 ( Z 깊이값 없는, 불완전한 투영행렬변환 메트릭스
FORCEINLINE Matrix4x4 CameraObject::GetPerspectiveMatrix() const
{
	float invA = 1.f / _ViewportSize.AspectRatio();
	float d = 1.f / tanf(Math::Deg2Rad(_FOV) * 0.5f);

...
void SoftRenderer::Update3D(float InDeltaSeconds)
{
	...
	Vector3 inputVector = Vector3(input.GetAxis(InputAxis::XAxis), input.GetAxis(InputAxis::YAxis), 
...
#endif
#if  defined(EX_12_2)
	const Matrix4x4 pvMatrix = mainCamera.GetPerspertiveViewMatrix();
#endif
...
#if defined(EX_12_1) || defined(EX_12_2)
void SoftRenderer::DrawTriangle3D(std::vector<Vertex3D>& InVertices, const LinearColor& InColor, FillMode InFillMode)
{
	...
	Vector3 faceNormal = -edge1.Cross(edge2); // ※ NDC 변환 후 왼손좌표계, 왼선 법칙을 따르기 때문에 - 처리

...
void SoftRenderer::Render3D()
{
	...
	for (auto it = g.SceneBegin(); it != g.SceneEnd(); ++it)
	{
		...
		{
			intersectedObjects++;
			DrawMeshClipping3D(mesh, finalMatrix, gameObject.GetColor());
		}
		else
		{
...
#ifdef EX_13_7
void SoftRenderer::DrawMeshClipping3D(const DDD::Mesh& InMesh, const Matrix4x4& InMatrix, const LinearColor& InColor)
{
	size_t vertexCount = InMesh.GetVertices().size();
	size_t indexCount = InMesh.GetIndices().size();
	size_t triangleCount = indexCount / 3;
	...
	for (int ti = 0; ti < triangleCount; ti++)
	{
		...
		if (useHomogeneousClipping) // @@@@ 이부분만 보면 됨. 그외에는 기존 DrawMesh3D 함수와 같음 @@@
		{

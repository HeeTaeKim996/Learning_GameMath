...
#if defined(EX_10_1) || defined(EX_10_2)
	// 플레이어 설정
...
void SoftRenderer::Update3D(float InDeltaSeconds)
{
	...
	else
	{
		...
		//goPlayer.GetTransform().AddPitchRotation(input.GetAxis(InputAxis::ZAxis) * rotateSpeed * InDeltaSeconds);
	}
	r.Roll += input.GetAxis(InputAxis::XAxis) * rotateSpeed * InDeltaSeconds;
	//goPlayer.GetTransform().AddRollRotation(input.GetAxis(InputAxis::XAxis) * rotateSpeed * InDeltaSeconds);

	r.Yaw += input.GetAxis(InputAxis::YAxis) * rotateSpeed * InDeltaSeconds;
	//goPlayer.GetTransform().AddYawRotation(input.GetAxis(InputAxis::YAxis) * rotateSpeed * InDeltaSeconds);

	goPlayer.GetTransform().AddPosition(Vector3::UnitZ * input.GetAxis(InputAxis::WAxis) * rotateSpeed * InDeltaSeconds);

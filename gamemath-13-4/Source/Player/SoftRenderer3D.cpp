...
void SoftRenderer::Render3D()
{
...
#if defined(EX_13_5) || defined(EX_13_6)
	...
	for (auto it = g.SceneBegin(); it != g.SceneEnd(); ++it)
	{
		...
		// PV(TRS) 메트릭스를 통해 로컬공간에서의 절두체 판정식 생성
		Matrix4x4 finalTransposedMatrix = finalMatrix.Transpose();
		std::array<Plane, 6> frustumPlanesFromMatrix = {
			Plane(finalTransposedMatrix[0] - finalTransposedMatrix[3]),
			Plane(-finalTransposedMatrix[0] - finalTransposedMatrix[3]),
			Plane(finalTransposedMatrix[1] - finalTransposedMatrix[3]),
			Plane(-finalTransposedMatrix[1] - finalTransposedMatrix[3]),
			Plane(finalTransposedMatrix[2] - finalTransposedMatrix[3]),
			Plane(-finalTransposedMatrix[2] - finalTransposedMatrix[3])
		};
		...
		BoundCheckResult checkResult = frustumFromMatrix.CheckBound(boxBound);
		if (checkResult == BoundCheckResult::Outside)
		{
			culledObjects++;
			continue;

...
FORCEINLINE constexpr BoundCheckResult Frustum::CheckBound(const Box& InBox) const
{
	...
	{
		Vector3 pPoint = InBox.Min, nPoint = InBox.Max;
		if (p.Normal.X >= 0) { pPoint.X = InBox.Max.X; nPoint.X = InBox.Min.X; }
		if (p.Normal.Y >= 0) { pPoint.Y = InBox.Max.Y; nPoint.Y = InBox.Min.Y; }
		if (p.Normal.Z >= 0) { pPoint.Z = InBox.Max.Z; nPoint.Z = InBox.Min.Z; }

		if (p.Distance(nPoint) > 0)
		{
			return BoundCheckResult::Outside;
		}
		if (p.Distance(nPoint) <= 0 && p.Distance(pPoint) >= 0)
		{
			return BoundCheckResult::Intersect;
		}
	}

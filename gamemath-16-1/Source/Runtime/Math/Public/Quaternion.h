...
FORCEINLINE constexpr void Quaternion::FromRotator(const Rotator& InRotator)
{
	float cy = 0.f, cp = 0.f, cr = 0.f;
	float sy = 0.f, sp = 0.f, sr = 0.f;
...
#if 1 // 공부
FORCEINLINE Quaternion Quaternion::Slerp(const Quaternion& startQuaternion, const Quaternion& endQuaternion,
	float InRatio)
{
	Quaternion q = startQuaternion;
	// 사원수 내적 구하기
	float dot = q.X * endQuaternion.X + q.Y * endQuaternion.Y + q.Z * endQuaternion.Z + q.W * endQuaternion.W;
	...
	Quaternion result;
	result.X = alpha * q.X + beta * endQuaternion.X;
	result.Y = alpha * q.Y + beta * endQuaternion.Y;
	result.Z = alpha * q.Z + beta * endQuaternion.Z;
	result.W = alpha * q.W + beta * endQuaternion.W;

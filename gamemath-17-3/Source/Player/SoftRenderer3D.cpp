...
void SoftRenderer::Update3D(float InDeltaSeconds)
{
	...
	goPlayer.GetTransform().AddLocalPosition(goPlayer.GetTransform().GetLocalZ()
		* input.GetAxis(InputAxis::YAxis) * moveSpeed * InDeltaSeconds);
	...
	for (auto it = g.SceneBegin(); it != g.SceneEnd(); ++it)
	{
		...
		if (mesh.IsSkinnedMesh() && IsWireframeDrawing())
		{
			...
			for (const auto& b : mesh.GetBones())
			{
				...
				// ※ GetWorldTransform 이지만, 지역공간에서의 WorldTransform 이지, 사실상 LocalTransform
		...
		/*	
			※ 위 if 문 내의 코드는, 실제 인게임에서 필요 없는, 본 을 그려주는 코드!
		*/
		...
		if (InMesh.IsSkinnedMesh())
		{
			...
			for (size_t wi = 0; wi < InMesh.GetConnectedBones()[vi]; wi++)
			{
				...
				if (InMesh.HasBone(boneName))
				{
					...
					const Transform& bindPose = b.GetBindPose(); // ※ GetBindPose : 최초 본의 트랜스폼 정보

					// 바인드 포즈를 중심으로 본의 로컬 공간을 계산
					...
					// ※ 이부분을 이해를 못하겠다. w.VAlues 가 0이면, Vector4::Zero가 최종포지션이 될텐데
					//    그보다는 vertices[vi].Position * (1 - t) + Vector4(skinnedWorldPosition, true) * t; 
					//    (st. t = w.VAlues[wi]; 로 해야 하지 않나
				}
			}
